...
const ChatPanel = ({ isOpen, onClose, theme }: ChatPanelProps) => {
    ...
    try {
        // URL de l'API backend (ajustez selon votre configuration)
        const API_URL = process.env.REACT_APP_API_URL || 'http://localhost:3000';
        const response = await axios.post(`${API_URL}/bot/voice-bot`, { text: input });
        const newMessage = response.data;
        setMessages((prev) =>
            prev.map((msg) =>
            msg.id === userMessage.id
                ? { ...msg, text_bot: newMessage.text_bot, isLoading: false }
                : msg
            )
        );
        } catch (error) {
    ...
    const handleLogout = async () => {
        try {
            // Supprimer le token et rediriger vers la page d'authentification
